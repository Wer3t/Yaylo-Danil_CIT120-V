...
std::string Migratory_bird::GetBird(){
    return GetRinged()+" "+GetSpecies()+" "+ std::to_string(GetAge())+" "+std::to_string(GetHouseSquare())+" "+std::to_string(GetHouseHeight())+" "+std::to_string(GetHouseNumFeeders())+" "+GetHouseNest()+" "+GetSex()+" Месяц отлёта: "+std::to_string(GetMonthAway()+1)+" Месяц прилёта: "+std::to_string(GetMonthArrival()+1);
}
