...
int finder(D_LinkedList *list, int criterion)
{
	...
	case 2:
		...
		while (tmp)
		{
			if (strcmp(str_crit, tmp->species) == 0)
			{
				printf("\nИскомое значение было найденно в структуре №%d", i);
			}
			i++;
	...
	case 6:
		...
		while (tmp)
		{
			if (to_find == tmp->house.num_feeders)
			{
				printf("\nИскомое значение было найденно в структуре №%d", i);
			}
			i++;
